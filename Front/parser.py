...
def data():
    ...
    if str(message).startswith('{"cars":'):
        abc = str(message).replace("'", "")
        abc1 = abc.replace('b', '')

        data = json.loads(abc1)
        ...
        try:
            count = 0
            while True:
                x = (data['cars'][count][str(count)][0][0]) #x
                y = (data['cars'][count][str(count)][0][1]) #y
                rotate = (data['cars'][count][str(count)][1]) #rotate

                html += f'<div class="car_red" id="{count}" style="left: {x}px; top: {y}px; transform: rotate({rotate}deg);">'
...
def datamap():
    ...
    if message.startswith('{"nodes":'):
        ...
        for i in nodes:
            x = i[0][0]
            y = i[0][1]
            html += f'<div class="main_circle" id="{count}" style="left: {x}px; top: {y}px;">'
            count += 1
        
        for i in roads:
            print(roads[i])
            convert = literal_eval(str(roads[i]))
            for i in convert:
                print(convert[i]) 